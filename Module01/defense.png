...
- name: 'Error management and basic tests'\
  **guidelines:**\
  ...
  to check the implementation, a combinaison different that those possible in
  the subject, should, as specified, print a detailed error message and exit properly.
    - Book:
      ...
      - After the initialization of a correct Book object, verify:
        ...
        - calling add_recipe() try the following bad arguments.
          Each of them must declare an error and exit properly.
        ...
        - get_recipe_by_name()
          - Give an existing recipe as parameter, the function must printed it and
            returned an instance of it
          - Give a non existing recipe as parameter, the function must declare
...
- name: 'built-in method __init__'\
  **guidelines:** \
  ...
  - a tuple of two integers, (a, b), with a < b:
    vector of (b - a + 1) elements should be created with values
    laying between a and b included.
  - a tuple of two integers (a, b), with a > b:
    This is an error and should be properly handled by displaying an
    error message.
  - a tuple of two integers (a, a):
    vector with one element must be created containing the value a.
    ...
    **guidelines:**
    ...
    - Second, you have to perform the following test to verify the security
      ...
      - `bank1.add(account1)`
        ...
        account:
        - account with all the attributes of a correct account excepted
          the name/id/value/zip/addr.
        - a account with all the attributes of a correct account plus one
          extra attribute (to get an even number of attributes).
      - `bank1.transfer(acc1, acc2, 42)`
        ...
        - 'acc1' and 'acc2' are valid but there is not enough money on
          'acc1' to perform the operation.
        - One of the accounts 'acc1' and 'acc2' is not valid.